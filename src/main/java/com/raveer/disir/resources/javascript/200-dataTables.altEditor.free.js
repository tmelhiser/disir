					...
					.extend(
							altEditor.prototype,
							{
								...
								_setup : function() {
									...
									// add modal
									$('body')
											.append('<div class="modal fade" id="altEditor-modal" tabindex="-1" role="dialog"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title"></h4></div><div class="modal-body"><p></p></div><div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">Close</button><input type="submit" form="altEditor-form" class="btn btn-primary"></input></div></div></div></div>');
								...
								_openEditModal : function() {
									...
									// Assuming that the first defined column is
									// ID - Therefore skipping that
									// and starting at index 1, because we dont
									// wanna be able to change the ID.
									...
									for (var j = 0; j < columnDefs.length; j++) {
										...
										if (columnDefs[j].type.indexOf("select")>=0) {
											...
											for (var i = 0; i < columnDefs[j].options.length; i++) {
												// Assigning the selected value
												// of the <selected> option
												if (adata.data()[0][columnDefs[j].name].indexOf(columnDefs[j].options[i])) {
													...
													function() {
														...
														$('#altEditor-modal')
																.find(
																		'.modal-footer')
																.html(
																		"<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>Close</button><button type='button' data-content='remove' class='btn btn-primary' id='editRowBtn'>Submit</button>");
								...
								_editRowData : function() {
									...
									// Calling AJAX with data, tableObject,
									// command.
									updateJSON(comepleteJsonData, that,
											"editRow");
								},
													...
													function() {
														...
														$('#altEditor-modal')
																.find(
																		'.modal-footer')
																.html(
																		"<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>Close</button><button type='button'  data-content='remove' class='btn btn-danger' id='deleteRowBtn'>Delete</button>");
													});
								...
								_deleteRow : function() {
									...
									// tablerow
									for (var i = 0; i < dt.context[0].aoColumns.length; i++) {
										jsonDataArray[dt.context[0].aoColumns[i].id] = adata
												.data()[0][dt.context[0].aoColumns[i].data];
									}
									...
									for (var i = 1; i < dt.context[0].aoColumns.length; i++) {
										columnDefs
												.push({
													...
													type : dt.context[0].aoColumns[i].type,
													options : dt.context[0].aoColumns[i].options,
													required : dt.context[0].aoColumns[i].required ? true : false,
													msg : dt.context[0].aoColumns[i].errorMsg,
													hoverMsg : dt.context[0].aoColumns[i].hoverMsg,
													pattern : dt.context[0].aoColumns[i].pattern,
													special : dt.context[0].aoColumns[i].special
													...
													function() {
														...
														$('#altEditor-modal')
																.find(
																		'.modal-footer')
																.html(
																		"<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>Close</button><input type='submit'  data-content='remove' class='btn btn-primary' id='addRowBtn'></input>");
													});
								...
								_addRowData : function() {
									...
									// Adding the ID & value(metadata) of the
									// row to the JsonArray
									...
									// Adding the inputs from the modal to
									// JsonArray
								...
								_getExecutionLocationFolder : function() {
									...
									var jsFileDirectory = jsFilePath.substring(
											0, jsFilePath.lastIndexOf("/") + 1);
...
var initValidation = function() {
	...
	$('form[name="altEditor-form"] *').filter(':text')
			.each(
					function(i) {
						...
						if ($(this).attr("data-special") === "portRange") {
							...
							if ($(this).val().indexOf(":")>=0) {
								...
								if (num1 < num2) {
									if (ports[0].match($(this).attr("pattern"))
											&& ports[1].match($(this).attr(
													"pattern"))) {
										$(errorLabel).hide();
										$(errorLabel).empty();
									} else {
										$(errorLabel).html(
												$(this).attr("data-errorMsg"));
										$(errorLabel).show();
										errorcount++
									}
								} else {
						...
						} else if ($(this).attr("data-special") === "requiredNoSpaces") {
							...
							$(this).context;
							if ($(this).val().match("(^$)|(\s+$)")) {
								$(errorLabel).html("Required Field");
								$(errorLabel).show();
								errorcount++;
							} else {
								$(errorLabel).hide();
...
var updateJSON = function(data, tableObj, act) {
	...
	var jqxhr = $
			...
			.done(
					function(data) {
						...
						if (data.error) {
							...
							var message = '<div class="alert alert-danger" role="alert"><strong>Fail!</strong> '+ data.error +'</div>';
			...
			.fail(
					function(error) {
						$('#altEditor-modal .modal-body .alert').remove();
						var message = '<div class="alert alert-danger" role="alert"><strong>Error!</strong> Reponse code: '+ error.status + '</div>';
						$('#altEditor-modal .modal-body').append(message);
					});
}